using AresLuaExtend.Common.Editor;
using AresLuaExtend.Common.Editor.InspectorGUI;
...
namespace AresLuaExtend.HadesBinding.Editor
{
	...
	[CustomPropertyDrawer(typeof(HadesBindOptionsAttribute))]
	public class HadesBindOptionsDrawer : PropertyDrawer
	{
		private ReorderableList m_mvvmBindList;
		private HadesBinding m_extraMVVMBinding;
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			m_mvvmBindList.onAddCallback += list =>
			{
				...
				bindModeProp.intValue = (int)HadesOption.BindMode.ONE_TIME;
			};
			...
			m_mvvmBindList.drawElementCallback += (rect, index, active, focused) =>
			{
				...
				{
					List<string> names;
					if (bindModeProp.intValue == (int)HadesOption.BindMode.EVENT)
					{
			...
			m_mvvmBindList.onSelectCallback += list =>
			{
				...
				var option = mvvmProp.GetPropertyObject() as HadesOption;
				if (!(option.BindTarget is IMVVMAssetReference refGetter))
					return;

				var assetRef = refGetter.GetMVVMReference();
				// {  } 属性模式（Property Pattern）等同于 (assetRef is AssetReference && assetRef.GUIDValid == true)
				...
				m_extraMVVMBinding = go.GetComponent<HadesBinding>();
				if (!m_extraMVVMBinding && go.transform.childCount > 0)
				{
					m_extraMVVMBinding = go.transform.GetChild(0).GetComponent<HadesBinding>();
				}
			};
		}
	}
